...
import type { AlertPayload } from '@/lib/api';
...
import {
  ...
  ChevronDown, ChevronUp, Search, Clock, User, MessageSquare, X, RefreshCw,
} from 'lucide-react';
...
type ProcessStep = 'received' | 'investigating' | 'processing' | 'completed';
...
const PROCESS_STEPS: { id: ProcessStep; label: string; color: string; bg: string; desc: string }[] = [
  ...
  { id: 'completed',     label: '4. 완료',    color: 'text-emerald-700', bg: 'bg-emerald-50',  desc: '문제 해결 완료' },
];
...
export default function AlertsPage() {
  ...
  const [showProcessModal, setShowProcessModal] = useState<AlertPayload | null>(null);
  ...
  }, [severityFilter, showResolved, search]);
  ...
  const counts = {
    total: alerts.length,
    critical: alerts.filter(a => a.severity === 'critical' && a.process_step !== 'completed').length,
    warning: alerts.filter(a => a.severity === 'warning' && a.process_step !== 'completed').length,
    completed: alerts.filter(a => a.process_step === 'completed').length,
  };

  const stepIndex = (step: string) => PROCESS_STEPS.findIndex(s => s.id === step);

  const openProcessModal = (alert: AlertPayload) => {
    setProcessForm({
      step: (alert.process_step as ProcessStep) || 'received',
      assignee: alert.assignee || '미배정',
      comment: '',
    });
  ...
  const handleProcessUpdate = async () => {
    ...
    try {
      await alertsApi.process(showProcessModal.id, {
        ...
        assignee: processForm.assignee !== '미배정' ? processForm.assignee : undefined,
        process_comment: processForm.comment || undefined,
      });
  ...
  return (
    <DashboardLayout title="알림 관리" subtitle={`미처리 알림 ${counts.total - counts.completed}건`}>
      {/* Stats */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 mb-4 sm:mb-6">
        {[
          { label: '전체 알림', value: counts.total, color: 'text-slate-800', bg: 'bg-white' },
          ...
          { label: '처리 완료', value: counts.completed, color: 'text-emerald-700', bg: 'bg-emerald-50 border-emerald-200' },
        ].map(s => (
      ...
      <div className="space-y-3">
        {loading ? (
          ...
          <div className="py-20 text-center">
            ...
            <p className="text-slate-400 text-sm mt-1">알림이 발생하면 여기에 표시됩니다</p>
          </div>
        ) : alerts.map(alert => {
          const severityKey = alert.severity as keyof typeof SEVERITY_CONFIG;
          const conf = SEVERITY_CONFIG[severityKey] || SEVERITY_CONFIG.info;
          const Icon = SEVERITY_ICONS[alert.severity] || Info;
          const isCompleted = alert.process_step === 'completed';
          const stepInfo = PROCESS_STEPS.find(s => s.id === alert.process_step);
          ...
          return (
            <div
              ...
              className={cn(
                ...
                isCompleted ? 'border-slate-100 opacity-70' :
              ...
              <div className="p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {alert.message && <div className="text-xs text-slate-500 mt-0.5">{alert.message}</div>}
                    <div className="flex items-center gap-3 mt-1.5 text-xs text-slate-400 flex-wrap">
                      {alert.equipment_name && <span>{alert.equipment_name}</span>}
                      {alert.equipment_name && alert.company_name && <span>·</span>}
                    ...
                    {PROCESS_STEPS.map((step, idx) => (
                      <div key={step.id} className="flex items-center gap-1 flex-1">
                        <div className={cn(
                          ...
                          idx <= stepIndex(alert.process_step || 'received') ? (
                            alert.process_step === 'completed' ? 'bg-emerald-500' :
                            alert.process_step === 'processing' ? 'bg-amber-400' :
                            alert.process_step === 'investigating' ? 'bg-blue-500' : 'bg-slate-300'
                          ) : 'bg-slate-100'
                        )} />
              ...
              {isExpanded && (
                <div className="px-4 pb-4 border-t border-slate-50 pt-3">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2 text-xs text-slate-500">
                      ...
                      <div>
                        ...
                        <span className="ml-2 text-slate-400">{formatDateTime(alert.created_at)}</span>
                      </div>
                    </div>
                    {alert.process_updated_at && (
                      <div className="flex items-start gap-2 text-xs text-slate-500">
                        <div className={cn('w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0',
                          alert.process_step === 'completed' ? 'bg-emerald-500' :
                          alert.process_step === 'processing' ? 'bg-amber-400' : 'bg-blue-500'
                        )} />
                        <div>
                          <span className="font-medium text-slate-700">{stepInfo?.label}</span>
                          {alert.assignee && <span className="ml-1 text-blue-600">({alert.assignee})</span>}
                          <span className="ml-2 text-slate-400">{formatDateTime(alert.process_updated_at)}</span>
                          {alert.process_comment && (
                            <div className="mt-0.5 text-slate-600 bg-slate-50 p-2 rounded-lg">{alert.process_comment}</div>
                          )}
                        </div>
                      </div>
                    )}
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-3 text-xs text-slate-500">
                    <div><span className="text-slate-400">장비 ID</span><br />{alert.equipment_id || '-'}</div>
                ...
                <div className="space-y-2">
                  {PROCESS_STEPS.map((step, idx) => (
                    ...
                    >
                      ...
                      {idx <= stepIndex(showProcessModal.process_step || 'received') && (
            ...
            <div className="flex justify-end gap-3 px-5 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">
              ...
              <button
                ...
                className={cn(
                  ...
                  processForm.step === 'completed' ? 'bg-emerald-600 hover:bg-emerald-700' :
