          ...
          {(['all', 'preventive', 'corrective', 'emergency', 'inspection'] as const).map(type => (
            <button
              ...
              className={cn(
                ...
                typeFilter === type ? 'bg-blue-600 text-white' : 'bg-white text-slate-600 border border-slate-200'
              )}
            >
              {type === 'all' ? '전체 유형' : TYPE_CONFIG[type].label}
            </button>
          ))}
        </div>
        <button
      ...
      <div className="space-y-3">
        {filtered.map(record => {
          ...
          return (
            <div
              key={record.id}
              className="bg-white rounded-xl border border-slate-200 p-5 hover:shadow-sm transition-all cursor-pointer group"
            >
              <div className="flex items-start gap-4">
                <div className={cn('w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0', statusConf.bg)}>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    {record.cost && (
                      <div className="text-right flex-shrink-0">
                        ...
                        <div className="font-bold text-slate-800">{record.cost.toLocaleString('ko-KR')}원</div>
                      </div>
                    )}
                  </div>
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span>
                        {record.completedDate
                          ? `완료: ${formatDate(record.completedDate)}`
                          : `예정: ${formatDate(record.scheduledDate)}`}
                      </span>
                    </div>
                    {record.laborHours && (
                      <div className="flex items-center gap-1.5">
                        <Clock className="w-3.5 h-3.5 text-slate-400" />
                        <span>{record.laborHours}시간</span>
                      </div>
                    )}
                    {record.nextMaintenance && (
                      <div className="flex items-center gap-1.5">
                        <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
                        <span>다음 점검: {formatDate(record.nextMaintenance)}</span>
                      </div>
                    )}
                  </div>
                </div>
